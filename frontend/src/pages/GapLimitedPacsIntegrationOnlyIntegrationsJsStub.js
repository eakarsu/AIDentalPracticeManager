...
/**
 * Gap/CFS Feature Page: Limited PACS integration (only `integrations.js` stub).
 * Slug: limited-pacs-integration-only-integrations-js-stub
 * Backend: /api/gap-limited-pacs-integration-only-integrations-js-stub
 */
export default function LimitedPacsIntegrationOnlyIntegrationsJsStubPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-pacs-integration-only-integrations-js-stub', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Limited PACS integration (only `integrations.js` stub).</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Limited PACS integration (only `integrations.js` stub).</p>
