...
/**
 * Gap/CFS Feature Page: No calendar integration (despite scheduling).
 * Slug: no-calendar-integration-despite-scheduling
 * Backend: /api/gap-no-calendar-integration-despite-scheduling
 */
export default function NoCalendarIntegrationDespiteSchedulingPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-calendar-integration-despite-scheduling', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No calendar integration (despite scheduling).</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No calendar integration (despite scheduling).</p>
