...
/**
 * Gap/CFS Feature Page: `xrayCv` lacks AI diagnosis endpoint (`/diagnose-from-xray`).
 * Slug: xraycv-lacks-ai-diagnosis-endpoint-diagnose-from-xray
 * Backend: /api/gap-xraycv-lacks-ai-diagnosis-endpoint-diagnose-from-xray
 */
export default function XraycvLacksAiDiagnosisEndpointDiagnoseFromXrayPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-xraycv-lacks-ai-diagnosis-endpoint-diagnose-from-xray', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>`xrayCv` lacks AI diagnosis endpoint (`/diagnose-from-xray`).</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>`xrayCv` lacks AI diagnosis endpoint (`/diagnose-from-xray`).</p>
