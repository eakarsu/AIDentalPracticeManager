...
/**
 * Gap/CFS Feature Page: Treatment outcome prediction
 * Slug: treatment-outcome-prediction
 * Backend: /api/cf-treatment-outcome-prediction
 */
export default function TreatmentOutcomePredictionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-treatment-outcome-prediction', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Treatment outcome prediction</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Predict treatment success likelihood; recommend alternatives if low probability.</p>
