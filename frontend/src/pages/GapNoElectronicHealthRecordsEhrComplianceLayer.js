...
/**
 * Gap/CFS Feature Page: No electronic health records (EHR) compliance layer.
 * Slug: no-electronic-health-records-ehr-compliance-layer
 * Backend: /api/gap-no-electronic-health-records-ehr-compliance-layer
 */
export default function NoElectronicHealthRecordsEhrComplianceLayerPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-electronic-health-records-ehr-compliance-layer', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No electronic health records (EHR) compliance layer.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No electronic health records (EHR) compliance layer.</p>
