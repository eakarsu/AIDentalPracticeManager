...
/**
 * Gap/CFS Feature Page: Appointment no-show prediction
 * Slug: appointment-no-show-prediction
 * Backend: /api/cf-appointment-no-show-prediction
 */
export default function AppointmentNoShowPredictionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-appointment-no-show-prediction', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Appointment no-show prediction</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Identify likely no-shows; auto-send reminders or reschedule.</p>
