...
/**
 * Gap/CFS Feature Page: Patient risk stratification
 * Slug: patient-risk-stratification
 * Backend: /api/cf-patient-risk-stratification
 */
export default function PatientRiskStratificationPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-patient-risk-stratification', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Patient risk stratification</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>ML model predicts high-risk patients (no-show, treatment default, poor outcomes); trigger interventions.</p>
